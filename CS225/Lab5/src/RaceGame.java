...
public class RaceGame {
	...
	public static void main(String[] args) {
		...
		Racer1 racer1 = new Racer1("Urza");
		Racer2 racer2 = new Racer2("Fenix");
		Racer3 racer3 = new Racer3("Drek");
		Racer4 racer4 = new Racer4("Dijkstra");

		// 2. while the any racer has not cross the finish line
		while (racer1.getLocation() < RACE_DISTANCE && racer2.getLocation() < RACE_DISTANCE
				&& racer3.getLocation() < RACE_DISTANCE && racer4.getLocation() < RACE_DISTANCE) {
			racer1.move();
			racer2.move();
			racer3.move();
			racer4.move();
		...
		// 3. find out who won
		double[] scores = new double[] { racer1.getLocation(), racer2.getLocation(), racer3.getLocation(),
				racer4.getLocation() };
		...
		if (winnerCount == 1) {
			System.out.println("Racer " + names[winnerIndex] + " Won the race with a distance of: "
					+ scores[winnerIndex] + " after " + turn + " turns");
		} else {
			System.out.println("Its a tie!(" + winnerCount
					+ " racers crossed the finish line in the same turn) But racer " + names[winnerIndex]
					+ " Went the furthest with a distance of " + scores[winnerIndex] + " after " + turn + " turns");
		}
